...
We assume that class value explains all the dependence between the the different attributes. In other words, that the attributes for each observation are independent if we know the class value.
...
\answer[15em]{\textbf{Answer Box (2.)}:\\%
    % Fill in this box with your answer to part 1
Without the attribute independence assumption, the $P(X,Y,Z,...,N | C)$ does not decompose into $P(X|C) \cdot P(Y|C) \cdot P(Z|C) \cdot ... \cdot P(N|C)$.
}
...
\answer[15em]{\textbf{Answer Box (1.)}:\\%
    % Fill in this box with your answer to part 1
The model performs well with separating unrelated classes (e.g. `alt.atheism` from the rest). It seems to get confused between `comp.sys.ibm.pc.hardware` and `comp.sys.mac.pc.hardware`, though, which is not entirely surprising as those two are very similar. Ultimately, the model performed well in $87.8\%$ of cases
}
